...
//           <p className="text-xs text-slate-500 mb-4">Create a free account or sign in to buy this item.</p>
//           {err && <div className="bg-red-50 border border-red-100 text-red-600 text-xs p-2 rounded-lg mb-3 text-center">{err}</div>}
...
//           </div>
//           <button onClick={() => onLogin('guest', 'guest')} className="w-full py-2 border border-slate-200 rounded-xl text-slate-600 text-sm font-medium hover:bg-slate-50 transition flex items-center justify-center gap-2">
//             <Eye size={13} /> Continue as guest
//           </button>
//           <p className="text-center text-[10px] text-slate-400 mt-2">New here? Any valid email creates a free account.</p>
//         </div>
//       </div>
//     </div>
//   );
...
// // ─────────────────────────────────────────────
// // AUTH MODAL — shown when Sign In / Register button clicked
// // ─────────────────────────────────────────────
...
//   const switchTab = (tab) => {
//     setActiveTab(tab); setError(''); setEmail(''); setPassword(''); setConfirmPass(''); setName('');
//   };
...
//     if (activeTab === 'admin') {
//       if (email === MY_ADMIN_EMAIL && password === MY_ADMIN_PASSWORD) { onLogin('admin', email); }
...
//           {error && <div className="bg-red-50 text-red-600 border border-red-100 text-xs p-2.5 rounded-xl mb-3 text-center font-medium">{error}</div>}
...
//                 <input type="email" placeholder={activeTab === 'admin' ? 'admin@example.com' : 'your@email.com'} value={email} onChange={e => setEmail(e.target.value)}
//                   className="w-full pl-8 pr-4 py-2.5 border border-slate-200 rounded-xl text-slate-800 text-sm bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition" />
//               </div>
//             </div>
//             <div>
...
//                 <div className="relative">
//                   <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400"><Lock size={14} /></span>
//                   <input type={showConfirm ? 'text' : 'password'} placeholder="••••••••" value={confirmPass} onChange={e => setConfirmPass(e.target.value)}
//                     className="w-full pl-8 pr-9 py-2.5 border border-slate-200 rounded-xl text-slate-800 text-sm bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition" />
...
//               <button type="button" onClick={() => onLogin('guest', 'guest')} className="w-full py-2.5 rounded-xl border border-slate-200 text-slate-600 font-semibold text-sm hover:bg-slate-50 transition flex items-center justify-center gap-2">
...
//   const staticReviews = [];
//   const totalReviews = staticReviews.length + comments.length;
...
// // ─────────────────────────────────────────────
// // LOGIN PAGE  (pre-login public view)
// // ─────────────────────────────────────────────
...
//           onLogin={(role, email) => { setShowAuthModal(false); onLogin(role, email); }}
//         />
//       )}
...
//                       <span className="text-slate-900 font-normal text-2xl md:text-3xl">Your smart market in Cambodia</span>
//                     </h1>
//                   </div>
...
//                     <button onClick={() => switchSection('products')} className="hero-btn flex items-center gap-2 text-white font-bold text-sm px-5 py-2.5 rounded-xl shadow"
//                       style={{ background: 'linear-gradient(135deg, #0625d3 0%, #0320c0 55%, #0306a1 100%)' }}>
//                       <ShoppingBag size={3} /> Shop Now
//                     </button>
//                     <button onClick={() => switchSection('deals')} className="hero-btn flex items-center gap-2 bg-white/10 border border-white/30 text-white font-semibold text-sm px-5 py-2.5 rounded-xl"
//                       style={{ background: 'linear-gradient(135deg, #0625d3 0%, #0320c0 55%, #0306a1 100%)' }}>
//                       <Tag size={3} /> View Deals
//                     </button>
//                     <button onClick={() => switchSection('offers')} className="hero-btn flex items-center gap-2 border border-yellow-400/40 text-yellow-200 font-semibold text-sm px-5 py-2.5 rounded-xl"
//                       style={{ background: 'linear-gradient(135deg, #0625d3 0%, #0320c0 55%, #0306a1 100%)' }}>
//                       <Gift size={3} /> View Offers
//                     </button>
//                   </div> */}
...
//                       className="hero-btn flex items-center gap-2 font-extrabold text-sm px-7 py-3 rounded-xl shadow-lg border border-white/30"
...
//                       className="hero-btn flex items-center gap-2 font-extrabold text-sm px-7 py-3 rounded-xl shadow-lg border border-blue-300/40"
...
//                 {/* ── RIGHT: uploadable project logo ── */}
//                 <div className="w-full lg:w-[500px] flex-shrink-0 flex flex-col items-center justify-center">
//                   <label className="logo-drop cursor-pointer group relative flex flex-col items-center justify-center" title="Click to upload your ecommerce logo">
//                     <div className="relative">
//                       {/* Logo display */}
//                       <div className="w-100 h-100 rounded-full overflow-hidden shadow-2xl border-4 border-white/30 bg-white/10 flex items-center justify-center"
...
//                         <div className="logo-drop-hint flex flex-col items-center gap-2 text-white">
//                           {/* <Camera size={32} /> */}
...
//                         if (file) setHeroLogo(URL.createObjectURL(file));
//                       }}
//                     /> */}
//                     {/* <p className="text-[11px] text-white/50 mt-3 text-center">Click to upload your project logo</p> */}
//                   </label>
//                 </div>
//                 {/* ── END RIGHT ── */}
...
//     setPendingRole(role); setPendingEmail(userEmail);
...
//               </div>
//               <button onClick={handleLogout} className="p-1.5 bg-red-950/40 hover:bg-red-900 border border-red-900/50 rounded-lg text-red-400 transition" title="Logout Session">
//                 <LogOut size={14} />
//               </button>
//             </div>
...
// // ─── Shared localStorage key (same key UserRegister.jsx reads) ───────────────
// const USER_LEDGER_KEY = 'psp_registered_users';

// // ─── Save a newly registered user into the shared ledger ─────────────────────
// function saveRegisteredUser(name, email) {
//   try {
//     const existing = JSON.parse(localStorage.getItem(USER_LEDGER_KEY) || '[]');
...
//     localStorage.setItem(USER_LEDGER_KEY, JSON.stringify(existing));
//     // Notify UserRegister dashboard instantly (same-tab)
//     window.dispatchEvent(new Event('psp_user_sync'));
//   } catch {}
// }
...
//     if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { setErr('Enter a valid email.'); return; }
//     if (password.length < 6) { setErr('Password must be at least 6 characters.'); return; }
//     onLogin('user', email, email.split('@')[0]);
//   };
//   return (
//     <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(4px)' }}>
//       <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-fadeUp">
...
//     // ── REGISTER ── save name + email to ledger before logging in
//     if (activeTab === 'register') {
//       if (!name.trim())                                         { setError('Please enter your full name.'); return; }
//       if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))           { setError('Please enter a valid email address.'); return; }
//       if (password.length < 6)                                  { setError('Password must be at least 6 characters.'); return; }
//       if (password !== confirmPass)                             { setError('Passwords do not match.'); return; }
//       // ✅ Save real registration data
//       saveRegisteredUser(name.trim(), email.trim());
...
//   // ✅ Now receives name as 3rd argument
//   const handleLogin = (role, userEmail, userName) => {
//     // Save real register data: only when it's a genuine named registration
//     if (role === 'user' && userName && userName !== userEmail.split('@')[0]) {
//       saveRegisteredUser(userName, userEmail);
//     }
//     setPendingRole(role);
