...
//     try {
//       const url = userRole === 'admin' ? '/api/orders' : `/api/orders?email=${encodeURIComponent(userEmail)}`;
//       const res = await fetch(url);
//       if (res.ok) {
//         const data = await res.json();
//         setOrderHistory(data);
//         localStorage.setItem('psp_market_order_history', JSON.stringify(data));
//       } else {
...
//   useEffect(() => {
//     loadHistory();
//     const iv = setInterval(loadHistory, 3000); // 3 seconds interval is sufficient
//     return () => clearInterval(iv);
//   }, [loadHistory]);
...
//   const saveOrder = useCallback(async (invoice, history) => {
//     const existIdx = history.findIndex(
//       o => o.customerName.toLowerCase().trim() === invoice.customerName.toLowerCase().trim() &&
//            o.phone.trim() === invoice.phone.trim() &&
//            o.accountEmail === invoice.accountEmail
...
//         items: merged, 
//         subtotal: newSub, 
//         total: newSub + history[existIdx].shippingFee, 
//         date: invoice.date,
//         status: 'confirmed'
//       };
//     } else {
//       finalInvoice = { ...invoice, status: 'confirmed' };
//     }

//     // Save to local state and localStorage immediately for instant feedback
//     const updatedLocal = history.filter(o => o.id !== finalInvoice.id);
//     updatedLocal.unshift(finalInvoice);
//     localStorage.setItem('psp_market_order_history', JSON.stringify(updatedLocal));
...
//         console.log('Order synced to DB successfully:', result.order);
//         return result.order;
//       }
//     } catch (err) {
...
//     const finalize = async () => {
//       const finalInvoice = await saveOrder(pendingInvoice, orderHistory);
//       setInvoiceDetails(finalInvoice);
...
//   const handleDeleteHistoryItem = async (orderId) => {
//     if (userRole !== 'admin') { alert('Access Denied.'); return; }
//     if (!window.confirm(`Permanently delete Order #${orderId}?`)) return;
...
//         loadHistory(); // reload to sync back
//       }
//     } catch (err) {
//       console.error('Failed to delete order from database:', err);
//       alert('Network error. Failed to delete from database.');
//       loadHistory(); // reload to sync back
//     }
//   };
...
// ❌ Reply <code>/cancel_${invoiceData.id}</code> to reject order
// `.trim();
//   return await telegramAPI('sendMessage', { chat_id: TELEGRAM_CHAT_ID, text: msg, parse_mode: 'HTML' });
// };
...
//               className="w-7 h-7 rounded-full border border-gray-200 flex items-center justify-center hover:bg-gray-50 disabled:opacity-30 transition shadow-sm">
//               <ChevronLeft size={14} className="text-gray-600" />
//             </button>
...
//           const reviews   = product.reviews ?? Math.floor(10 + Math.random() * 290);
//           return (
//             <div key={product.id} className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 group flex flex-col">
//               <div className="relative overflow-hidden bg-gray-50 h-36">
...
//                 )}
//                 {inCart && !justAdded && (
//                   <span className="absolute top-2 right-2 bg-green-500 text-white text-[9px] font-bold px-2 py-0.5 rounded-full shadow-sm">In Cart</span>
//                 )}
//               </div>
//               <div className="p-3 flex flex-col flex-1">
//                 <h3 className="text-xs font-bold text-gray-900 line-clamp-2 leading-snug mb-1">{product.name}</h3>
...
//                     className={`flex items-center gap-1 text-[10px] font-bold px-3 py-1.5 rounded-xl transition shadow-sm
//                       ${justAdded ? 'bg-green-500 text-white' : inCart ? 'bg-blue-50 text-blue-600 border border-blue-200 hover:bg-blue-100' : 'bg-blue-700 hover:bg-blue-800 text-white'}`}>
//                     {justAdded ? <><CheckCircle2 size={10} /> Added!</> : inCart ? <><Plus size={10} /> Add More</> : <><Plus size={10} /> Add</>}
//                   </button>
//                 </div>
//               </div>
//             </div>
...
//     } catch (err) {
//       console.error('Failed to load order history from database:', err);
//       // Fallback to localStorage cache only if DB is down
//       try {
//         const saved = localStorage.getItem('psp_market_order_history');
...
//   useEffect(() => {
//     loadHistory();
//     // Poll every 5s for new orders while cart page is open
...
//   const saveOrderToDB = useCallback(async (invoice) => {
//     try {
//       const res = await fetch(`${API}/api/orders`, {
//         method: 'POST',
//         headers: { 'Content-Type': 'application/json' },
//         body: JSON.stringify({ ...invoice, status: 'confirmed' }),
//       });
//       if (!res.ok) throw new Error(`HTTP ${res.status}`);
//       const result = await res.json();
//       console.log('✓ Order synced to database:', result.order?.id);
//       return result.order;
//     } catch (err) {
//       console.error('✗ Failed to sync order to database:', err);
//       return invoice; // return the invoice even if DB fails so UI still works
//     }
//   }, []);
...
//     setProofError('');
//     const allowed = ['image/jpeg','image/png','image/webp','image/gif','application/pdf'];
//     if (!allowed.includes(file.type)) { setProofError('Only images (JPG, PNG, WebP) or PDF files are accepted.'); return; }
//     if (file.size > 10 * 1024 * 1024) { setProofError('File too large. Maximum size is 10 MB.'); return; }
//     setProofFile(file);
//     if (file.type !== 'application/pdf') {
...
//   const handleSubmitProof = async () => {
//     if (!proofFile) { setProofError('Please attach your payment screenshot or PDF before submitting.'); return; }
//     setPayStatus(PAY_STATUS.SUBMITTING);
//     setUploadProgress('Sending order details…');
//     try {
//       await sendOrderAlert(pendingInvoice);
//       setUploadProgress('Uploading payment proof…');
//       const proofResult = await sendPaymentProof(proofFile, pendingInvoice.id, pendingInvoice.customerName, pendingInvoice.total.toFixed(2));
//       if (!proofResult?.ok) throw new Error('File upload failed');
//       setUploadProgress('Waiting for admin confirmation…');
//       setPayStatus(PAY_STATUS.VERIFYING);
//       startPolling(pendingInvoice.id);
//     } catch (err) {
...
//       // Reload history from DB to reflect the new order
//       await loadHistory();
...
//   // ── Delete order from REAL DATABASE (admin only) ──────────────────────────
//   const handleDeleteHistoryItem = async (orderId) => {
//     if (userRole !== 'admin') { alert('Access Denied.'); return; }
//     if (!window.confirm(`Permanently delete Order #${orderId} from the database?`)) return;
...
//         alert(`Failed to delete from database: ${errData.message || 'Unknown error'}`);
//         await loadHistory(); // reload to restore accurate state
//       }
//     } catch (err) {
//       console.error('Failed to delete order from database:', err);
//       alert('Network error. Failed to delete from database.');
...
//                   <button onClick={() => setShowHistoryModal(true)}
//                     className="text-blue-600 hover:text-blue-700 font-bold text-xs flex items-center gap-1 bg-blue-50 py-1.5 px-3 rounded-lg transition">
//                     <History size={13} /> {userRole === 'admin' ? 'All Sales' : 'My History'} ({allowedHistory.length})
//                   </button>
//                 </div>
//                 <div className="divide-y divide-gray-100">
//                   {cart.map(item => (
...
//               <div className="bg-white border border-gray-200 p-6 rounded-2xl shadow-sm text-xs">
//                 <h2 className="font-black text-gray-900 uppercase tracking-wider border-b border-gray-100 pb-4 mb-4 flex items-center gap-2">
//                   <MapPin size={18} className="text-blue-700" /> Delivery Information
//                 </h2>
//                 {formError && (
//                   <div className="bg-red-50 text-red-600 p-3 rounded-xl mb-4 font-semibold text-center border border-red-100 flex items-center gap-2">
//                     <AlertCircle size={14} /> {formError}
...
//               <div className="bg-white border border-gray-200 p-6 rounded-2xl shadow-sm text-xs">
//                 <h2 className="font-black text-gray-900 uppercase tracking-wider border-b border-gray-100 pb-3 mb-4 flex items-center gap-2">
//                   <Truck size={16} className="text-blue-700" /> Delivery Carrier
//                 </h2>
//                 <div className="space-y-2">
//                   {deliveryServices.map(({ id, name, rate, eta, icon: Icon }) => {
//                     const sel = deliveryMethod === name;
...
//         ].slice(0, 12);
//         if (similar.length === 0) return null;
//         return <SimilarProducts products={similar} onAddToCart={addToCart} cartIds={cartIds} />;
//       })()}
...
//               <div className="flex items-center justify-center gap-2 mt-3">
//                 {['Scan & Pay', 'Upload Proof', 'Confirmed'].map((step, i) => {
//                   const stepStatuses = [[PAY_STATUS.PENDING],[PAY_STATUS.UPLOADING, PAY_STATUS.SUBMITTING],[PAY_STATUS.VERIFYING, PAY_STATUS.CONFIRMED]];
//                   const isActive = stepStatuses[i].includes(payStatus);
//                   const isPast = (
//                     (i === 0 && [PAY_STATUS.UPLOADING, PAY_STATUS.SUBMITTING, PAY_STATUS.VERIFYING, PAY_STATUS.CONFIRMED].includes(payStatus)) ||
//                     (i === 1 && [PAY_STATUS.VERIFYING, PAY_STATUS.CONFIRMED].includes(payStatus))
...
//                     <React.Fragment key={step}>
//                       <div className="flex flex-col items-center">
//                         <div className={`w-5 h-5 rounded-full text-[9px] font-black flex items-center justify-center border-2 transition ${isPast ? 'bg-white border-white text-blue-700' : isActive ? 'bg-blue-500 border-white text-white' : 'bg-transparent border-blue-400 text-blue-300'}`}>
//                           {isPast ? '✓' : i + 1}
//                         </div>
//                         <span className={`text-[8px] mt-0.5 font-bold ${isActive || isPast ? 'text-white' : 'text-blue-400'}`}>{step}</span>
//                       </div>
...
//                   <div className="mb-3">
//                     <div className="flex justify-between items-center mb-1">
//                       <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wide flex items-center gap-1"><Timer size={10} /> QR Expires In</span>
//                       <span className={`font-black text-sm font-mono ${timerUrgent ? 'text-red-600 animate-pulse' : 'text-gray-800'}`}>{timerMM}:{timerSS}</span>
//                     </div>
//                     <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
//                       <div className={`h-full rounded-full transition-all duration-1000 ${timerUrgent ? 'bg-red-500' : 'bg-blue-500'}`} style={{ width: `${timerPct}%` }} />
//                     </div>
//                   </div>
//                   <div className="bg-white border-2 border-gray-100 rounded-2xl p-4 flex items-center justify-center mb-3 shadow-inner">
//                     <img src={khqrImage} alt="KHQR" className="w-52 h-52 object-contain" />
...
//                   </p>
//                   <div className="flex gap-3 text-xs font-bold">
//                     <button onClick={handleCancelPayment} className="w-2/5 border border-gray-200 text-gray-500 py-3 rounded-xl hover:bg-gray-50 transition">Cancel</button>
//                     <button onClick={handleUserConfirm} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl shadow-md transition flex items-center justify-center gap-2">
//                       <CheckCircle2 size={14} /> I Have Transferred
//                     </button>
//                   </div>
//                 </>
...
//                     <ShieldCheck size={14} className="text-amber-600 mt-0.5 shrink-0" />
//                     <p className="text-[11px] text-amber-800 font-semibold leading-relaxed">
//                       Please attach a screenshot or PDF of your Bakong transfer confirmation. This is sent directly to the shop owner for verification.
//                     </p>
//                   </div>
//                   {!proofFile ? (
...
//                   ) : (
//                     <div className="border border-green-200 bg-green-50 rounded-2xl p-3 mb-3 relative">
//                       <button onClick={handleRemoveProof} className="absolute top-2 right-2 bg-white border border-gray-200 rounded-full p-0.5 hover:bg-red-50 hover:border-red-300 transition">
//                         <X size={12} className="text-gray-500" />
//                       </button>
//                       {proofPreview ? (
//                         <img src={proofPreview} alt="Payment proof" className="w-full max-h-40 object-contain rounded-xl mb-2" />
...
//                   )}
//                   <div className="flex gap-3 text-xs font-bold">
//                     <button onClick={() => setPayStatus(PAY_STATUS.PENDING)} className="w-2/5 border border-gray-200 text-gray-500 py-3 rounded-xl hover:bg-gray-50 transition">← Back</button>
//                     <button onClick={handleSubmitProof} disabled={!proofFile}
//                       className={`flex-1 py-3 rounded-xl shadow-md transition flex items-center justify-center gap-2 ${proofFile ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
//                       <Send size={13} /> Submit Proof
//                     </button>
...
//                       <span className="text-[11px] font-bold text-green-800">Receipt delivered to admin</span>
//                     </div>
//                     <p className="text-[10px] text-green-700 font-mono">Auto-checking for approval every 4s…</p>
//                   </div>
//                   <div className="flex items-center justify-center gap-1 mb-4">
//                     <Timer size={11} className={timerUrgent ? 'text-red-500' : 'text-gray-400'} />
//                     <span className={`font-mono font-black text-sm ${timerUrgent ? 'text-red-600 animate-pulse' : 'text-gray-600'}`}>{timerMM}:{timerSS} remaining</span>
//                   </div>
//                   <button onClick={handleCancelPayment} className="w-full border border-gray-200 text-gray-400 py-2.5 rounded-xl hover:bg-gray-50 font-bold text-xs">Cancel Order</button>
//                 </div>
//               )}
...
//                   <p className="text-gray-400 text-xs mb-4">The payment window has closed. Generate a new QR to try again.</p>
//                   <div className="flex gap-3 text-xs font-bold">
//                     <button onClick={handleCancelPayment} className="flex-1 border border-gray-200 text-gray-500 py-3 rounded-xl">Cancel</button>
//                     <button onClick={handleRetry} className="flex-1 bg-amber-500 hover:bg-amber-600 text-white py-3 rounded-xl flex items-center justify-center gap-1.5">
//                       <RefreshCw size={12} /> New QR Code
//                     </button>
//                   </div>
//                 </div>
...
//                   <h3 className="font-black text-red-700 text-sm mb-1">Order Rejected</h3>
//                   <p className="text-gray-400 text-xs mb-4">The shop owner could not verify this payment. Please contact support or try again.</p>
//                   <button onClick={handleCancelPayment} className="w-full border border-gray-200 text-gray-600 py-3 rounded-xl font-bold text-xs">Close</button>
//                 </div>
//               )}
//             </div>
//           </div>
...
//                 <CheckCircle2 className="h-9 w-9 text-green-500 mx-auto mb-2" />
//                 <h2 className="text-xl font-black tracking-wide text-gray-900 uppercase">PSP MART INVOICE</h2>
//                 <p className="text-gray-400 text-[10px]">Payment Verified · Saved to Database</p>
//               </div>
//               <div className="grid grid-cols-2 gap-y-2 py-4 border-b border-gray-100 text-[11px]">
//                 <div>
//                   <span className="text-gray-400 block">Invoice ID</span>
//                   <button onClick={() => copyInvoiceId(invoiceDetails.id)} className="font-bold text-gray-900 flex items-center gap-1 hover:text-blue-600">
//                     #{invoiceDetails.id}
//                     {copyFeedback === invoiceDetails.id ? <CheckCircle2 size={10} className="text-green-500" /> : <Copy size={10} />}
//                   </button>
//                 </div>
...
//                 <div className="text-right"><span className="text-gray-400 block">Phone</span><span className="font-medium text-gray-900">{invoiceDetails.phone}</span></div>
//                 <div><span className="text-gray-400 block">Carrier</span><span className="font-bold text-blue-600">{invoiceDetails.carrier}</span></div>
//                 <div className="text-right"><span className="text-gray-400 block">Shipping</span><span className="font-bold text-gray-900">${Number(invoiceDetails.shippingFee).toFixed(2)}</span></div>
//                 <div className="col-span-2"><span className="text-gray-400 block">Account</span><span className="font-medium text-blue-600">{invoiceDetails.accountEmail}</span></div>
//                 <div className="col-span-2"><span className="text-gray-400 block">Address</span><span className="font-medium text-gray-900">{invoiceDetails.address}</span></div>
//                 {invoiceDetails.mapLocation && (
//                   <div className="col-span-2">
//                     <a href={invoiceDetails.mapLocation} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-blue-500 hover:underline font-medium">
//                       <ExternalLink size={10} /> View Map Route
//                     </a>
//                   </div>
//                 )}
//               </div>
//               <div className="py-3 border-b border-dashed border-gray-300">
//                 <span className="block font-black uppercase tracking-wider text-gray-400 text-[9px] mb-2">Items</span>
//                 <div className="max-h-[160px] overflow-y-auto space-y-2 pr-1">
//                   {(invoiceDetails.items || []).map((item, idx) => (
//                     <div key={idx} className="flex justify-between items-center text-[11px] bg-slate-50 p-1.5 rounded-lg border border-slate-100/60">
//                       <span className="font-medium text-gray-900">{item.name} <span className="text-blue-600 font-bold">×{item.quantity}</span></span>
...
//               <div className="pt-4 flex justify-between items-center">
//                 <span className="text-xs font-black uppercase tracking-wider text-gray-900">Total Paid</span>
//                 <span className="text-xl font-black text-green-600">${Number(invoiceDetails.total).toFixed(2)}</span>
//               </div>
//             </div>
//             <div className="mt-5 grid grid-cols-2 gap-3 font-bold text-xs">
...
//                 <Share2 size={14} /> Share Invoice
//               </button>
//               <button onClick={() => setShowInvoiceModal(false)} className="col-span-2 mt-1 border border-dashed border-gray-300 text-gray-400 text-center py-2.5 rounded-xl hover:bg-gray-50">Close</button>
//             </div>
//           </div>
//         </div>
//       )}
...
//               <h3 className="text-base font-black text-gray-900 flex items-center gap-2 uppercase tracking-wide">
//                 <Receipt size={18} className="text-blue-700" />
//                 {userRole === 'admin' ? 'All Sales (Admin)' : 'My Purchase History'}
//               </h3>
//               <div className="flex items-center gap-2">
//                 <span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-md font-bold">{filteredHistory.length} records</span>
//                 <button onClick={loadHistory} disabled={historyLoading} className="flex items-center gap-1 text-xs text-blue-600 hover:text-blue-700 font-bold disabled:opacity-50">
...
//             <div className="relative my-2">
//               <Search size={14} className="absolute left-3 top-3.5 text-gray-400" />
//               <input type="text" placeholder="Search by Invoice ID, Name, or Phone…" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
//                 className="w-full pl-9 pr-4 py-2.5 text-xs bg-slate-50 border border-gray-200 rounded-xl focus:border-blue-500 focus:bg-white outline-none transition" />
//               {searchQuery && (
...
//                       <span className="text-[10px] text-gray-400 flex items-center gap-1 mt-0.5"><Clock size={10} /> {order.date}</span>
//                     </div>
//                     <span className="font-black text-blue-700 text-sm">${Number(order.total).toFixed(2)}</span>
//                   </div>
//                   <div className="text-gray-500 space-y-0.5 border-t border-gray-200/60 pt-2 text-[11px]">
//                     {userRole === 'admin' && (
//                       <div className="text-red-600 font-bold"><span className="text-gray-700 font-semibold">Email:</span> {order.accountEmail}</div>
...
//                       {(order.items || []).reduce((s, i) => s + i.quantity, 0)} item(s) · {order.carrier}
//                     </div>
//                   </div>
//                   <div className="mt-3 flex gap-2">
//                     <button onClick={() => { setInvoiceDetails(order); setShowInvoiceModal(true); }}
...
//                   <div className="text-[8pt] text-gray-600 leading-relaxed">
//                     <div>{invoiceDetails.address}</div>
//                     {invoiceDetails.mapLocation && <div className="text-blue-600 text-[7.5pt] mt-1 break-all">{invoiceDetails.mapLocation}</div>}
//                   </div>
//                 </div>
//                 <div className="bg-blue-50 border border-blue-200 rounded p-4">
//                   <div className="text-[7pt] font-black text-blue-400 uppercase tracking-widest mb-2">Payment Method</div>
...
//               </div>
//               <div className="mb-6">
//                 <div className="grid bg-blue-900 text-white text-[8pt] font-black uppercase tracking-wider rounded-t px-3 py-2.5" style={{ gridTemplateColumns: '1fr 60px 90px 90px' }}>
//                   <div>Description</div><div className="text-center">Qty</div><div className="text-right">Unit Price</div><div className="text-right">Amount</div>
//                 </div>
//                 {(invoiceDetails.items || []).map((item, i) => (
//                   <div key={i} className={`grid px-3 py-2.5 text-[9pt] border-l border-r border-b border-gray-200 items-center ${i % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`}
//                     style={{ gridTemplateColumns: '1fr 60px 90px 90px' }}>
//                     <div className="font-semibold text-gray-900">{item.name}</div>
//                     <div className="text-center text-gray-600 font-semibold">{item.quantity}</div>
//                     <div className="text-right text-gray-600">${Number(item.price).toFixed(2)}</div>
...
//                 <div className="w-64">
//                   <div className="flex justify-between text-[8.5pt] text-gray-500 py-1.5 border-b border-gray-100">
//                     <span>Subtotal</span><span className="font-semibold text-gray-800">${Number(invoiceDetails.subtotal).toFixed(2)}</span>
//                   </div>
//                   <div className="flex justify-between text-[8.5pt] text-gray-500 py-1.5 border-b border-gray-100">
//                     <span>Shipping ({invoiceDetails.carrier})</span><span className="font-semibold text-gray-800">${Number(invoiceDetails.shippingFee).toFixed(2)}</span>
//                   </div>
//                   <div className="flex justify-between text-[8.5pt] text-green-600 py-1.5 border-b border-gray-100">
//                     <span>Discount</span><span className="font-semibold">$0.00</span>
//                   </div>
//                   <div className="flex justify-between items-center bg-blue-900 text-white px-4 py-3 rounded mt-2">
//                     <span className="text-[8pt] font-black uppercase tracking-wider text-blue-200">Total Paid</span>
//                     <span className="text-[15pt] font-black">${Number(invoiceDetails.total).toFixed(2)}</span>
//                   </div>
//                 </div>
//               </div>
//               <div className="bg-gray-50 border border-gray-200 rounded p-4 mb-8">
...
// 🚚 <b>Carrier:</b> ${invoiceData.carrier} (+$${Number(invoiceData.shippingFee).toFixed(2)})
// ${mapLine}
...
// 📦 Shipping: $${Number(invoiceData.shippingFee).toFixed(2)}
// 💰 <b>TOTAL: $${Number(invoiceData.total).toFixed(2)}</b>
...
// 🆔 Invoice: <code>#${invoiceData.id}</code>
// 👤 ${invoiceData.customerName} (${invoiceData.phone})
// 💰 <b>$${Number(invoiceData.total).toFixed(2)} RECEIVED</b>
// 🚚 ${invoiceData.carrier}
// ━━━━━━━━━━━━━━━━━━━━━
// 🚀 Proceed to dispatch & fulfill order.
// `.trim();
...
//   // ── Save confirmed order to REAL DATABASE ─────────────────────────────────
//   const saveOrderToDB = useCallback(async (invoice) => {
//     try {
//       const res = await fetch(`${API}/api/orders`, {
//         method: 'POST',
...
//       await sendOrderAlert(pendingInvoice);
//       setUploadProgress('Uploading payment proof…');
//       const proofResult = await sendPaymentProof(proofFile, pendingInvoice.id, pendingInvoice.customerName, Number(pendingInvoice.total).toFixed(2));
//       if (!proofResult?.ok) throw new Error('File upload failed');
//       setUploadProgress('Waiting for admin confirmation…');
//       setPayStatus(PAY_STATUS.VERIFYING);
//       startPolling(pendingInvoice.id);
...
//         const errData = await res.json().catch(() => ({}));
//         alert(`Failed to delete from database: ${errData.message || 'Unknown error'}`);
//         await loadHistory(); // restore accurate state
//       }
//     } catch (err) {
//       console.error('Failed to delete order from database:', err);
//       alert('Network error. Failed to delete from database.');
...
//   const handleShareInvoice = async () => {
//     if (!invoiceDetails) return;
//     const text = `PSP Mart Invoice #${invoiceDetails.id}\nTotal: $${Number(invoiceDetails.total).toFixed(2)}\nCustomer: ${invoiceDetails.customerName}`;
//     if (navigator.share) {
//       try { await navigator.share({ title: 'PSP Mart Receipt', text, url: window.location.href }); } catch {}
//     } else {
//       navigator.clipboard.writeText(text);
...
//                   {historyLoading ? '…' : `${filteredHistory.length} records`}
//                 </span>
//                 <button onClick={loadHistory} disabled={historyLoading}
//                   title="Refresh from server"
//                   className="flex items-center gap-1 text-xs text-blue-600 hover:text-blue-700 font-bold disabled:opacity-50 bg-blue-50 px-2 py-1 rounded-lg transition">
//                   <RefreshCw size={11} className={historyLoading ? 'animate-spin' : ''} />
...
//                     <div className="text-center text-gray-600 font-semibold">{item.quantity}</div>
//                     <div className="text-right text-gray-600">${Number(item.price).toFixed(2)}</div>
//                     <div className="text-right font-bold text-blue-900">${(Number(item.price) * item.quantity).toFixed(2)}</div>
//                   </div>
//                 ))}
//                 <div className="h-1 bg-blue-900 rounded-b" />
//               </div>
...
//   const [pendingInvoice,     setPendingInvoice]      = useState(null);
//   const [lastTelegramUpdate, setLastTelegramUpdate]  = useState(null);
//   const [invoiceFromHistory, setInvoiceFromHistory] = useState(false);
//   const [copyFeedback,       setCopyFeedback]        = useState('');
...
//                   <button onClick={() => copyInvoiceId(invoiceDetails.id)} className="font-bold text-gray-900 flex items-center gap-1 hover:text-blue-600">
//                     #{invoiceDetails.id}
//                     {copyFeedback === String(invoiceDetails.id) ? <CheckCircle2 size={10} className="text-green-500" /> : <Copy size={10} />}
//                   </button>
//                 </div>
//                 <div className="text-right"><span className="text-gray-400 block">Date</span><span className="font-medium text-gray-600">{invoiceDetails.date}</span></div>
//                 <div><span className="text-gray-400 block">Customer</span><span className="font-bold text-gray-900">{invoiceDetails.customerName}</span></div>
...
//                   if (invoiceFromHistory) {
//                     setInvoiceFromHistory(false);
//                     setShowHistoryModal(true);
//                   } else {
//                     // came from a new payment — show history so they see the new order
//                     setShowHistoryModal(true);
//                   }
//                 }}
//                 className="col-span-2 mt-1 border border-dashed border-gray-300 text-gray-400 text-center py-2.5 rounded-xl hover:bg-gray-50 flex items-center justify-center gap-1.5">
//                 <History size={12} /> View in Order History
//               </button>
//             </div>
//           </div>
//         </div>
...
//                   </div>
//                   <div className="mt-3 flex gap-2">
//                     <button onClick={() => { setInvoiceDetails(order); setInvoiceFromHistory(true); setShowHistoryModal(false); setShowInvoiceModal(true); }}
//                       className="flex-1 bg-white border border-blue-200 text-blue-600 hover:bg-blue-50 font-bold py-2 rounded-xl text-[11px] shadow-sm transition flex items-center justify-center gap-1.5">
//                       <Receipt size={13} /> View Invoice
//                     </button>
//                     {userRole === 'admin' && (
...
//   // ── Invoice image state ────────────────────────────────────────────────────
//   // Stores the base64 proof image for the currently-viewed invoice
//   const [invoiceProofImage,      setInvoiceProofImage]      = useState(null);
//   const [invoiceProofLoading,    setInvoiceProofLoading]    = useState(false);
//   const [invoiceProofIsPdf,      setInvoiceProofIsPdf]      = useState(false);
...
//   // ── Fetch proof image from DB when opening an invoice from history ─────────
//   const loadInvoiceProofImage = useCallback(async (orderId) => {
//     setInvoiceProofImage(null);
//     setInvoiceProofIsPdf(false);
//     setInvoiceProofLoading(true);
//     try {
//       const res = await fetch(`${API}/api/invoice-images/${orderId}`);
//       if (!res.ok) {
//         // 404 just means no proof was saved — not an error
//         setInvoiceProofImage(null);
//         return;
//       }
//       const data = await res.json();
//       if (data?.image) {
//         setInvoiceProofImage(data.image);
//         // Detect PDF by base64 prefix
//         setInvoiceProofIsPdf(data.image.startsWith('data:application/pdf'));
//       }
//     } catch (err) {
//       console.error('Failed to load invoice proof image:', err);
//       setInvoiceProofImage(null);
//     } finally {
//       setInvoiceProofLoading(false);
//     }
//   }, []);

//   // ── Save proof image to DB backend ────────────────────────────────────────
//   const saveInvoiceImageToDB = useCallback(async (orderId, accountEmail, file) => {
//     try {
//       const base64 = await fileToBase64(file);
//       const res = await fetch(`${API}/api/invoice-images`, {
//         method: 'POST',
//         headers: { 'Content-Type': 'application/json' },
//         body: JSON.stringify({ orderId, accountEmail, image: base64 }),
//       });
//       if (!res.ok) throw new Error(`HTTP ${res.status}`);
//       const result = await res.json();
//       console.log('✓ Invoice image saved to DB:', result.id);
//       return base64; // return base64 so we can display it immediately
//     } catch (err) {
//       console.error('✗ Failed to save invoice image to DB:', err);
//       return null;
//     }
//   }, []);
...
//   // ── Submit proof: send to Telegram AND save to DB ─────────────────────────
//   const handleSubmitProof = async () => {
//     if (!proofFile) { setProofError('Please attach your payment screenshot or PDF before submitting.'); return; }
//     setPayStatus(PAY_STATUS.SUBMITTING);
//     setUploadProgress('Sending order details…');
...
//       // ── NEW: Save proof image to backend DB so it can be retrieved later ──
//       setUploadProgress('Saving proof to records…');
//       await saveInvoiceImageToDB(pendingInvoice.id, pendingInvoice.accountEmail, proofFile);
...
//       setInvoiceDetails(savedOrder || confirmedInvoice);
//       setInvoiceFromHistory(false);
//       // Clear the proof image state so the new invoice modal starts fresh
//       setInvoiceProofImage(null);
//       setInvoiceProofIsPdf(false);
...
//   // ── Open invoice from history: load invoice + fetch proof image from DB ────
//   const handleViewInvoiceFromHistory = useCallback((order) => {
//     setInvoiceDetails(order);
//     setInvoiceFromHistory(true);
//     setShowHistoryModal(false);
//     setInvoiceProofImage(null);
//     setInvoiceProofIsPdf(false);
//     setShowInvoiceModal(true);
//     // Fetch the proof image asynchronously — shows loading indicator in modal
//     loadInvoiceProofImage(order.id);
//   }, [loadInvoiceProofImage]);
...
//       {showInvoiceModal && invoiceDetails && (
//         <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[60] print:hidden">
//           <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl border border-gray-100 max-h-[90vh] overflow-y-auto">
//             <div className="text-xs text-gray-800">
//               <div className="text-center pb-4 border-b border-dashed border-gray-300">
//                 <CheckCircle2 className="h-9 w-9 text-green-500 mx-auto mb-2" />
//                 <h2 className="text-xl font-black tracking-wide text-gray-900 uppercase">PSP MART INVOICE</h2>
...
//                 </div>
//               </div>
//               <div className="pt-4 flex justify-between items-center border-b border-gray-100 pb-4">
//                 <span className="text-xs font-black uppercase tracking-wider text-gray-900">Total Paid</span>
//                 <span className="text-xl font-black text-green-600">${Number(invoiceDetails.total).toFixed(2)}</span>
//               </div>
...
//               <div className="pt-4">
//                 <span className="block font-black uppercase tracking-wider text-gray-400 text-[9px] mb-2 flex items-center gap-1">
//                   <ImagePlus size={10} /> Payment Proof
//                 </span>
//                 {invoiceProofLoading ? (
//                   <div className="flex items-center justify-center gap-2 py-6 bg-slate-50 rounded-xl border border-slate-100">
//                     <RefreshCw size={13} className="animate-spin text-blue-500" />
//                     <span className="text-[11px] text-gray-400 font-medium">Loading proof image…</span>
//                   </div>
//                 ) : invoiceProofImage ? (
//                   invoiceProofIsPdf ? (
//                     /* PDF proof — show a download link since we can't render PDF inline */
//                     <a
//                       href={invoiceProofImage}
//                       download={`proof-${invoiceDetails.id}.pdf`}
//                       className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl p-3 hover:bg-red-100 transition"
//                     >
//                       <div className="w-10 h-10 bg-red-100 rounded-xl flex items-center justify-center shrink-0">
//                         <FileText size={18} className="text-red-600" />
//                       </div>
//                       <div>
//                         <p className="text-xs font-bold text-gray-800">Payment Proof (PDF)</p>
//                         <p className="text-[10px] text-blue-600 font-medium flex items-center gap-1 mt-0.5">
//                           <Download size={9} /> Click to download
//                         </p>
//                       </div>
//                     </a>
//                   ) : (
//                     /* Image proof — show full preview with lightbox-style zoom */
//                     <div className="rounded-xl overflow-hidden border border-gray-200 shadow-sm">
//                       <img
//                         src={invoiceProofImage}
//                         alt="Payment proof"
//                         className="w-full object-contain max-h-64 bg-gray-50 cursor-zoom-in"
//                         onClick={() => window.open(invoiceProofImage, '_blank')}
//                         title="Click to open full size"
//                       />
//                       <div className="bg-gray-50 border-t border-gray-100 px-3 py-2 flex items-center justify-between">
//                         <span className="text-[10px] text-gray-500 flex items-center gap-1">
//                           <CheckCircle2 size={10} className="text-green-500" /> Proof on file
//                         </span>
//                         <a
//                           href={invoiceProofImage}
//                           download={`proof-${invoiceDetails.id}.jpg`}
...
//                   <div className="flex items-center gap-2 py-4 px-3 bg-slate-50 rounded-xl border border-dashed border-slate-200">
//                     <AlertCircle size={13} className="text-gray-300 shrink-0" />
//                     <span className="text-[11px] text-gray-400 italic">No payment proof on record for this order.</span>
//                   </div>
//                 )}
//               </div>
...
//                   </div>
//                   <div className="mt-3 flex gap-2">
//                     {/* ── Use the new handler that also loads the proof image ── */}
//                     <button
//                       onClick={() => handleViewInvoiceFromHistory(order)}
//                       className="flex-1 bg-white border border-blue-200 text-blue-600 hover:bg-blue-50 font-bold py-2 rounded-xl text-[11px] shadow-sm transition flex items-center justify-center gap-1.5">
//                       <Receipt size={13} /> View Invoice
//                     </button>
//                     {userRole === 'admin' && (
...
//                 </div>
//               </div>
//               {/* ── Proof image in print layout (only if available and is an image) ── */}
//               {invoiceProofImage && !invoiceProofIsPdf && (
//                 <div className="mb-8">
//                   <div className="text-[7pt] font-black text-gray-400 uppercase tracking-widest mb-2">Payment Proof</div>
//                   <img src={invoiceProofImage} alt="Payment proof" className="max-h-48 object-contain border border-gray-200 rounded" />
//                 </div>
//               )}
//               <div className="bg-gray-50 border border-gray-200 rounded p-4 mb-8">
//                 <div className="text-[7pt] font-black text-gray-400 uppercase tracking-widest mb-2">Notes &amp; Terms</div>
...
// ─── FRONTEND ORDER HYDRATOR ─────────────────────────────────────────────────
// Normalizes DB snake_case ↔ frontend camelCase and ensures correct types
function hydrateOrderFE(order) {
  if (!order) return null;

  // Parse items if it came back as a JSON string
  ...
  return {
    ...
    accountEmail: order.accountEmail ?? order.account_email ?? '',
    customerName: order.customerName ?? order.customer_name ?? '',
    mapLocation:  order.mapLocation  ?? order.map_location  ?? null,
    carrier:      order.carrier      ?? 'Standard Home Delivery',
    phone:        order.phone        ?? '',
    address:      order.address      ?? '',
    date:         order.date         ?? order.date_str ?? (order.created_at
      ? new Date(order.created_at).toLocaleString('en-US', { timeZone: 'Asia/Phnom_Penh' })
      : ''),
...
export default function Cart({ userEmail, userRole }) {
  ...
  const loadInvoiceProofImage = useCallback(async (orderId) => {
    ...
    try {
      const res = await fetch(`${API}/api/invoice-images/${orderId}`);
      if (!res.ok) {
        // 404 = no proof saved — not an error
        setInvoiceProofImage(null);
        return;
      }
      const data = await res.json();
  ...
  const saveOrderToDB = useCallback(async (invoice) => {
    try {
      ...
      const result = await res.json();
      console.log('✓ Order synced to database:', result.order?.id);
      // ✅ FIX: Always hydrate the saved order before returning
      return result.order ? hydrateOrderFE(result.order) : null;
    } catch (err) {
      console.error('✗ Failed to sync order to database:', err);
      return null;
    }
  ...
  // ── When admin confirms via Telegram → finalize and save to DB ────────────
  useEffect(() => {
    if (payStatus !== PAY_STATUS.CONFIRMED || !pendingInvoice) return;
    ...
    const finalize = async () => {
      ...
      // ✅ FIX: hydrateOrderFE applied inside saveOrderToDB already;
      //         fall back to hydrateOrderFE(confirmedInvoice) if save failed
      setInvoiceDetails(savedOrder ?? hydrateOrderFE(confirmedInvoice));
      setInvoiceFromHistory(false);
      setInvoiceProofImage(null);
      setInvoiceProofIsPdf(false);
  ...
  // ── Delete order from database (admin only) ───────────────────────────────
  const handleDeleteHistoryItem = async (orderId) => {
    if (userRole !== 'admin') { alert('Access Denied.'); return; }
    if (!window.confirm(`Permanently delete Order #${orderId} from the database?`)) return;
  ...
  // ── ✅ FIX: Open invoice from history — hydrate order + load proof image ───
  const handleViewInvoiceFromHistory = useCallback((order) => {
    // Always hydrate so items/fields are correct regardless of DB format
    const hydrated = hydrateOrderFE(order);
    setInvoiceDetails(hydrated);
    setInvoiceFromHistory(true);
    setShowHistoryModal(false);
    setInvoiceProofImage(null);
    setInvoiceProofIsPdf(false);
    setShowInvoiceModal(true);
    // Fetch proof image using the normalized id
    loadInvoiceProofImage(hydrated.id);
  }, [loadInvoiceProofImage]);
              ...
              {historyLoading && orderHistory.length === 0 ? (
                ...
                <div key={order.id} className="bg-slate-50/70 p-4 border border-slate-100 rounded-xl hover:border-blue-300 transition shadow-sm">
                  ...
                  <div className="text-gray-500 space-y-0.5 border-t border-gray-200/60 pt-2 text-[11px]">
                    ...
                    <div><span className="font-semibold text-gray-700">Customer:</span> {order.customerName} ({order.phone})</div>
                    <div className="text-[11px] text-slate-500 italic mt-1">
                      {(order.items || []).reduce((s, i) => s + (Number(i.quantity) || 1), 0)} item(s) · {order.carrier}
                    </div>
                  </div>
                  <div className="mt-3 flex gap-2">
                    {/* ✅ FIX: handleViewInvoiceFromHistory hydrates order + loads proof */}
                    <button
                      onClick={() => handleViewInvoiceFromHistory(order)}
                      className="flex-1 bg-white border border-blue-200 text-blue-600 hover:bg-blue-50 font-bold py-2 rounded-xl text-[11px] shadow-sm transition flex items-center justify-center gap-1.5">
                      <Receipt size={13} /> View Invoice
